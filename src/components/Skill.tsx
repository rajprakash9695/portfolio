...
const Skill: React.FC = () => {
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-5 my-20">
      <p className=" text-lg sm:text-xl font-['Poppins'] text-end">Skills</p>
      ...
      <div className="my-32  max-w-screen-lg mx-auto slider-container px-5">
        <Slider {...settings}>
          {SkillData.map((item, index) => (
            <div key={item.key} className="">
              <img
                src={item.img}
                alt={item.alt}
                className="flex mx-auto max-w-96 "
              />
            </div>
          ))}
        </Slider>
